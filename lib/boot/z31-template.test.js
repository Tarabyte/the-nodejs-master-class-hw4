...
module.exports = async (app, config) => {
  await test.module('Template', async () => {
    await test('Basic template loading and interpolation', async () => {
      const tmpl = await template('templates/test_101.html')
    ...
    await test('Including template', async () => {
      const tmpl = await template('templates/test_including.html')
    ...
    await test('Simultaneous rendering', async () => {
      assert.strictEqual(
        await template('templates/test_101.html', { test: 'Something' }),
        'Something'
      )
    })
  })
