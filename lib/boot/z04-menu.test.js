...
module.exports = async (app, config) => {
  ...
  const users = fetch({ path: '/api/users', port: config.http.port })
  const menu = fetch({ path: '/api/products', port: config.http.port })
  const tokens = fetch({ path: '/api/tokens', port: config.http.port })
