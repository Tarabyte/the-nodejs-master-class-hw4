...
module.exports = async (app, config) => {
  ...
  const users = options =>
    fetch({ path: '/users', port: config.http.port, ...options })

  const tokens = options =>
    fetch({ path: '/tokens', port: config.http.port, ...options })
  ...
  await test('Editing current user', async () => {
    ...
    // updating user
    const { statusCode: successfulStatusCode, data: updatedUser } = await users(
      {
        method: 'PUT',
        query: {
          id: userData.email,
