...
const bye = require('./web/accountDeleted')
const orders = require('./web/orders')
const cart = require('./web/cart')
const checkout = require('./web/checkout')
...
const app = {
  ...
  async start(config) {
    try {
      // tell everyone we are booting the app
      app.events.emit('starting', config)
      ...
      app.events.emit('started')
    } catch (e) {
      notify.red('Unable to boot application')

      app.events.emit('error', e)
