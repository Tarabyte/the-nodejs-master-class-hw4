...
class Collection {
   ...
   */
  async read(id) {
    const itemId = typeof id === 'object' ? this.getId(id) : id

    debug('%s: Reading item by id %s.', this.name, itemId)
    const fileName = this._getFileName(itemId)
  ...
  async patch(itemId, updates) {
    ...
    // apply updates
    const updated =
      typeof updates === 'function' ? updates(item) : { ...item, ...updates }

    // save
    const saved = await this.update(updated)
