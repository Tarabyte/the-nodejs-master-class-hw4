...
const mix = (arr1, arr2) =>
  Array.from(
    ...
    (_, i) => (i % 2 ? arr2 : arr1)[(i / 2) | 0]
  ).join('')

// template helpers
const helpers = {
  url(strings, ...values) {
    return mix(strings, values.map(value => encodeURIComponent(value)))
  },
}
...
// inject helper factory
const injectFactory = templatePath => {
  const dir = dirname(templatePath)

  return path => load(join(dir, path))
}

// create template local context
const localFactory = (storage = new Map()) =>
  function(name, value) {
...
const co = async (strings, ...values) => mix(strings, await Promise.all(values))
...
// loade files and compiled templates caches
const templatesCache = cache()
const filesCache = cache()
...
`
    ...
    const inject = injectFactory(path)
    ...
    const tmpl = (localData, localHelpers) =>
      fn(
        ...
        prepareData(
          { ...helpers, ...localHelpers, include, local, inject },
    ...
    templatesCache.set(path, tmpl)

    return tmpl
  } catch (e) {
    // simplify wrond template path detection
    if (e.code === 'ENOENT') {
